...
export function drawVehicle(
  ...
  switch (kind) {
    ...
    case 'camel': {
      ...
      // גוף
      drawBox(ctx, cam, x + s * 0.14, y + s * 0.26, s * 0.66, s * 0.4, 0.22, faceColors(coat, 'leather'), bodyZ + gait);
      if (kind === 'camel') {
        // דבשת
        drawBox(ctx, cam, x + s * 0.34, y + s * 0.32, s * 0.28, s * 0.28, 0.14, faceColors(shade(coat, 0.05), 'leather'), bodyZ + 0.22 + gait);
      }
      ...
      drawBox(ctx, cam, hx - s * 0.08, y + s * 0.37, s * 0.18, s * 0.16, 0.1, faceColors(shade(coat, 0.08), 'leather'), bodyZ + 0.34 + gait);
      // זנב
      ...
      drawBox(ctx, cam, x + s * 0.34, y + s * 0.38, s * 0.2, s * 0.18, 0.24, faceColors(owner, 'cloth'), rz);
    ...
    case 'tank': {
      const hull = '#6b7050';
      // שרשראות
      drawBox(ctx, cam, x, y, s, s * 0.22, 0.12, faceColors('#3a3d33', 'metal'));
      drawBox(ctx, cam, x, y + s * 0.54, s, s * 0.22, 0.12, faceColors('#3a3d33', 'metal'));
      // גוף משופע
      drawBox(ctx, cam, x + s * 0.04, y + s * 0.16, s * 0.92, s * 0.46, 0.2, faceColors(hull, 'metal'), 0.08);
      // צריח
      drawBox(ctx, cam, x + s * 0.26, y + s * 0.24, s * 0.44, s * 0.32, 0.16, faceColors(shade(hull, 0.06), 'metal'), 0.28);
    ...
    case 'apc': {
      const hull = '#777a56';
      drawBox(ctx, cam, x, y, s, s * 0.2, 0.11, faceColors('#3a3d33', 'metal'));
      drawBox(ctx, cam, x, y + s * 0.56, s, s * 0.2, 0.11, faceColors('#3a3d33', 'metal'));
      drawBox(ctx, cam, x + s * 0.05, y + s * 0.14, s * 0.9, s * 0.5, 0.3, faceColors(hull, 'metal'), 0.07);
      ownerStripe(ctx, cam, x, y + s * 0.14, s, s * 0.5, 0.37, owner);
      // צריחון מקלע
      drawBox(ctx, cam, x + s * 0.4, y + s * 0.3, s * 0.2, s * 0.18, 0.1, faceColors(shade(hull, 0.1), 'metal'), 0.37);
      break;
    }
    case 'jeep': {
      const body = shade(owner, -0.1);
      drawBox(ctx, cam, x + s * 0.06, y + s * 0.18, s * 0.88, s * 0.44, 0.18, faceColors(body, 'metal'), 0.06);
      drawBox(ctx, cam, x + s * 0.3, y + s * 0.22, s * 0.34, s * 0.36, 0.14, faceColors(shade(body, -0.2), 'metal'), 0.24);
    ...
    case 'aa': {
      const hull = '#6d7360';
      drawBox(ctx, cam, x + s * 0.05, y + s * 0.18, s * 0.9, s * 0.46, 0.22, faceColors(hull, 'metal'), 0.06);
      for (const [wxo, wyo] of [[0.18, 0.14], [0.78, 0.14], [0.18, 0.66], [0.78, 0.66]]) {
        drawColumn(ctx, cam, x + s * wxo, y + s * wyo, 0.06, 0.1, '#2b2b2b');
      }
      // תאי יירוט משופעים
      drawBox(ctx, cam, x + s * 0.28, y + s * 0.26, s * 0.42, s * 0.3, 0.26, faceColors(shade(hull, -0.1), 'metal'), 0.28);
      for (let i = 0; i < 3; i++) {
    ...
    case 'drone': {
      ...
      const body = '#c9cdd2';
      drawBox(ctx, cam, x + s * 0.3, y + s * 0.3, s * 0.4, s * 0.2, 0.1, faceColors(body, 'metal'), z);
      // כנפיים
      poly(ctx, [
    ...
    case 'catapult': {
      drawBox(ctx, cam, x + s * 0.1, y + s * 0.25, s * 0.8, s * 0.4, 0.14, faceColors('#7a5c3a', 'timber'), 0.06);
    ...
    case 'ram': {
      drawBox(ctx, cam, x + s * 0.08, y + s * 0.2, s * 0.84, s * 0.5, 0.16, faceColors('#6b4a2f', 'timber'), 0.05);
      // גגון
      poly(ctx, [
    ...
    case 'ballista': {
      drawBox(ctx, cam, x + s * 0.15, y + s * 0.3, s * 0.7, s * 0.34, 0.12, faceColors('#7a5c3a', 'timber'), 0.06);
    ...
    case 'chariot': {
      drawBox(ctx, cam, x + s * 0.3, y + s * 0.28, s * 0.4, s * 0.34, 0.2, faceColors(shade(owner, -0.05), 'cloth'), 0.08);
      for (const wyo of [0.2, 0.7]) {
        drawColumn(ctx, cam, x + s * 0.42, y + s * wyo, 0.1, 0.06, '#5a4026');
      }
      // סוס מקורב
      drawBox(ctx, cam, x + s * (dir > 0 ? 0.78 : -0.1), y + s * 0.34, s * 0.34, s * 0.22, 0.24, faceColors('#8a6a45', 'timber'), 0.1);
      break;
    }
  }
}
